...
# вспомогательная функция, показывает коллайдеры всех спрайтов в заданной группе
def ShowCollide(Group, w):
    for Spr in Group.sprites():
        pygame.draw.rect(w, (0, 200, 64), Spr.collider, 3)

# Lesson 6
#функция определяющая пересекаются ли два спрайта
...
class Player(Sprite):
    ...
    def update(self, keys, g, GrPlatf, GrFireBall):
        ...
        if collideG(self.rect, GrFireBall) and not self.shock:
            self.health -= 10
            soundbabah.play()
            GrFireBall.sprites()[GrFireBall.indexcollide].state = 1
            if self.health <= 0:
                Stage = 2
            self.shock = True
...
class Fireball(Sprite):
    def __init__(self, img, group):
        ...
        self.cadr = 0
        self.state = 0
        self.imgbabah = pygame.image.load('Images/fireballbabah.png')
...
# Подготавливаем саундтрек игры
soundtrack = pygame.mixer.Sound('music/soundtrack.wav')
soundtrack.play()
soundtrack.set_volume(0.1)

soundbabah = pygame.mixer.Sound('music/babah.wav')
soundjump = pygame.mixer.Sound('music/jump.wav')


# Начинаем игровой цикл
game = True
while game:
    #   цикл будет крутиться 24 раза в секунду, то есть это частота кадров игры
    clock.tick(24)

    # пробегаемся по списку событий, если нажат крестик закрытия окна приложения, выходим из программы
    for ev in pygame.event.get ():
        if ev.type == pygame.QUIT:
            game = False

    #запрашиваем список нажатых клавиш, даем ему имя - keys
    keys = pygame.key.get_pressed()

    # start menu
    if Stage == 0:
        buttonstart.update()
        w.blit(menu.image, menu.rect)
        w.blit(buttonstart.image, buttonstart.rect)
