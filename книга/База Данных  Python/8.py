...
def getrow():
    cursor.execute('SELECT Learner, Class, Subject, Teacher FROM learns INNER JOIN teachers ON learns.Teacher = teachers.id')
...
def addrow():
    ...
    s += Erat.get() + ', '
    s += Epart.get()
    cursor.execute('''INSERT INTO learns
                   (id, Learner, Class, Teacher, Subject, Rate, Part)
                   VALUES (''' + s + ')')
    con.commit()
